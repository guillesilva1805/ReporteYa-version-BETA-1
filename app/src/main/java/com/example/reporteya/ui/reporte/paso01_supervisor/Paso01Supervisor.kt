...
fun Paso01Supervisor(onValidity: (Boolean) -> Unit) {
    ...
    var seleccionado by remember(estado.value.supervisor) { mutableStateOf(estado.value.supervisor.orEmpty()) }

    val context = LocalContext.current
    LaunchedEffect(Unit) {
        cargando = true; error = null
        val url = "https://uppdkjfjxtjnukftgwhz.supabase.co/rest/v1/supervisores?select=*"
        // primero intenta leer cache
                    ...
                    DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                        opciones.forEach { item ->
                            DropdownMenuItem(text = { Text(item) }, onClick = {
                                seleccionado = item
                                respuestas_reporte.actualizar { r -> r.copy(supervisor = item) }
                                expanded = false
                                onValidity(true)
                            })
                        }
